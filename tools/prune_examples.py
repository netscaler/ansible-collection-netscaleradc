...
Why this exists
---------------
The ``netscaler.adc`` metadata (modules, map, examples) is produced by an external
code generator and dropped into the collection as a versioned snapshot folder
(e.g. ``13_4/``). That snapshot's ``copy`` step ``cp -f``'s every ``examples/*.yaml``
into the collection. The example generator, however, emits example YAMLs for a
broader set of NITRO resources than it generates *modules* for -- binding-only
globals (``systemglobal``), namespaces (``ns``, ``system``), read-only resources
(``systemcpu``, ``nsconnectiontable``), and resources deliberately dropped as
faulty/ambiguous (``urlfilteringparameter``). Those "orphan" examples reference a
``netscaler.adc.<resource>`` module that does not exist, so ``ansible-lint`` /
``--syntax-check`` over ``examples/`` fails with "couldn't resolve module/action".

This tool deletes exactly those orphans: an example is pruned when it references a
``netscaler.adc.<module>`` for which no ``plugins/modules/<module>.py`` exists (and
which is not a ``module_defaults`` action group). Hand-written demonstration
playbooks (``basic-content-switching.yaml``, ``save_config.yaml``, ...) that only
use real modules are always kept, regardless of their file name.

Run it after every refresh (``make prune_examples`` from the collection root), or
with ``--check`` in CI to fail if any orphan slipped in.
"""
