...
def list_modules(module_dir, depth=0):
    ...
    for module_path in files:
        ...
        if filename in ( '__init__.py', 'netscaler.py'):
            continue
        category = categories
        ...
        for new_cat in ['network', 'netscaler']:
...
def main():
    ...
    import json
    print('mod_info %s' % json.dumps(mod_info, indent=4))
    print('categories %s' % json.dumps(categories, indent=4))
    ...
    #
    print('category_names %s' % json.dumps(category_names))
    print('categories %s' % json.dumps(categories))
    for category in category_names:
        print('category %s' % json.dumps(category))
