...
def populate_template(template_dir, **kwargs):
    ...
    template = env.get_template('citrix_adm_rba_role.template')
    stream = template.stream(**kwargs)

    #output_file = 'netscaler_appfw_profile.py'
    output_file = os.path.join(HERE, '..', '..', '..', 'ansible-modules', 'citrix_adm_rba_role.py')
    stream.dump(
        output_file,
        encoding='utf-8'
    )
...
def main():
    ...
    main_yaml_file = os.path.join(yaml_dir, 'rba_role.yaml')
    ...
    attributes_config_list.append(
        calculate_attributes_config_dict('rba_role', main_object_attributes)
    )
