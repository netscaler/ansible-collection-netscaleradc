...
def populate_template(template_dir, **kwargs):
    ...
    template = env.get_template('citrix_adm_dns_domain_entry.template')
    stream = template.stream(**kwargs)

    #output_file = 'netscaler_appfw_profile.py'
    output_file = os.path.join(HERE, '..', '..', '..', 'ansible-modules', 'citrix_adm_dns_domain_entry.py')
    stream.dump(
        output_file,
        encoding='utf-8'
    )
...
def main():
    ...
    main_yaml_file = os.path.join(yaml_dir, 'dns_domain_entry.yaml')
    ...
    main_data = calculate_attributes_config_dict('dns_domain_entry', main_object_attributes)
    ...
    populate_template(
        ...
        main_nitro_class='dns_domain_entry',
