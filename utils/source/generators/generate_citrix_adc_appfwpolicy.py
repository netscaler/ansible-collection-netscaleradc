...
def populate_template(template_dir, **kwargs):
    ...
    template = env.get_template('citrix_adc_appfwpolicy.template')
    stream = template.stream(**kwargs)

    #output_file = 'citrix_adc_appfw_profile.py'
    output_file = os.path.join(HERE, '..', '..', '..', 'ansible-modules', 'citrix_adc_appfw_policy.py')
    stream.dump(
        output_file,
        encoding='utf-8'
    )
...
def main():
    ...
    main_json_file = os.path.join(json_dir, 'appfwpolicy.json')
    ...
    attributes_config_list.append(
        calculate_attributes_config_dict('appfwpolicy', main_object_attributes, skip_attributes=['newname'])
    )
