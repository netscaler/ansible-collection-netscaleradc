...
def populate_template(template_dir, **kwargs):
    ...
    template = env.get_template('citrix_adm_tenant_facts.template')
    stream = template.stream(**kwargs)

    #output_file = 'netscaler_appfw_profile.py'
    output_file = os.path.join(HERE, '..', '..', '..', 'ansible-modules', 'citrix_adm_tenant_facts.py')
    stream.dump(
        output_file,
        encoding='utf-8'
    )
...
def main():
    ...
    main_yaml_file = os.path.join(yaml_dir, 'tenant.yaml')
