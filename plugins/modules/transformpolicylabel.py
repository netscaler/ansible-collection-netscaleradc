...
DOCUMENTATION = r"""
---
module: transformpolicylabel
short_description: Configuration for transform policy label resource.
description: Configuration for transform policy label resource.
version_added: 2.0.0
author:
  ...
  labelname:
    ...
    description:
      - Name for the policy label. Must begin with a letter, number, or the underscore
        ...
        characters. Can be changed after the URL Transformation policy label is added.
      - ''
      - 'The following requirement applies only to the Citrix ADC CLI:'
      - If the name includes one or more spaces, enclose the name in double or single
        quotation marks (for example, my transform policylabel or my transform policylabel).
  newname:
    type: str
    description:
  ...
  policylabeltype:
    ...
    description:
      - Types of transformations allowed by the policies bound to the label. For URL
        transformation, always C(http_req) (HTTP Request).
  transformpolicylabel_transformpolicy_binding:
    type: dict
    description: Bindings for transformpolicylabel_transformpolicy_binding resource
    suboptions:
      mode:
        type: str
        default: desired
