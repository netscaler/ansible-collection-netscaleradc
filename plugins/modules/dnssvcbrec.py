#!/usr/bin/python

# -*- coding: utf-8 -*-

# Copyright (c) 2025 Cloud Software Group, Inc.
# MIT License (see LICENSE or https://opensource.org/licenses/MIT)

from __future__ import absolute_import, division, print_function

__metaclass__ = type


ANSIBLE_METADATA = {
    "metadata_version": "1.1",
    "status": ["preview"],
    "supported_by": "community",
}

DOCUMENTATION = r"""
---
module: dnssvcbrec
short_description: Configuration for SVCB/HTTPS service binding record resource.
description: Configuration for SVCB/HTTPS service binding record resource.
version_added: 2.0.0
author:
  - Sumanth Lingappa (@sumanth-lingappa)
  - Shiva Shankar Vaddepally (@shivashankar-vaddepally)
options:
  state:
    choices:
      - present
      - absent
      - unset
    default: present
    description:
      - The state of the resource being configured by the module on the NetScaler
        ADC node.
      - When C(present), the resource will be added/updated configured according to
        the module's parameters.
      - When C(absent), the resource will be deleted from the NetScaler ADC node.
      - When C(unset), the resource will be unset on the NetScaler ADC node.
    type: str
  remove_non_updatable_params:
    choices:
      - 'yes'
      - 'no'
    default: 'no'
    description:
      - When given yes, the module will remove any parameters that are not updatable
        in the resource.
      - If no, the module will return error if any non-updatable parameters are provided.
    type: str
  alpn:
    type: str
    description:
      - Comma-separated list of ALPN protocol identifiers.
  domain:
    type: str
    description:
      - Domain name for the SVCB/HTTPS record.
  encryptedclienthello:
    type: str
    description:
      - Base64-encoded ECH configuration.
  ipv4hint:
    type: str
    description:
      - Comma-separated list of IPv4 hint addresses.
  ipv6hint:
    type: str
    description:
      - Comma-separated list of IPv6 hint addresses.
  mandatory:
    type: str
    description:
      - Comma-separated list of mandatory SvcParam keys.
  nodefaultalpn:
    type: bool
    description:
      - Indicates no default ALPN protocols.
  nodeid:
    type: int
    description:
      - Unique number that identifies the cluster node.
  port:
    type: int
    description:
      - Port number for the service.
  priority:
    type: int
    description:
      - Service priority (0 for AliasMode, >0 for ServiceMode).
  svcbtype:
    type: str
    choices:
      - SVCB
      - HTTPS
    description:
      - 'Service type: C(SVCB) or C(HTTPS).'
  targetname:
    type: str
    description:
      - Target domain name.
  ttl:
    type: int
    description:
      - Time to Live (TTL) in seconds.
  type:
    type: str
    choices:
      - ALL
      - ADNS
      - PROXY
    description:
      - 'Type of records: C(ADNS), C(PROXY), or C(ALL).'
extends_documentation_fragment: netscaler.adc.netscaler_adc

"""

EXAMPLES = r"""
"""

RETURN = r"""
---
changed:
  description: Indicates if any change is made by the module
  returned: always
  type: bool
  sample: true
diff:
  description: Dictionary of before and after changes
  returned: always
  type: dict
  sample: {'before': {'key1': 'xyz'}, 'after': {'key2': 'pqr'}, 'prepared': 'changes
      done'}
diff_list:
  description: List of differences between the actual configured object and the configuration
    specified in the module
  returned: when changed
  type: list
  sample: ["Attribute `key1` differs. Desired: (<class 'str'>) XYZ. Existing: (<class
      'str'>) PQR"]
failed:
  description: Indicates if the module failed or not
  returned: always
  type: bool
  sample: false
loglines:
  description: list of logged messages by the module
  returned: always
  type: list
  sample: ['message 1', 'message 2']

"""


import os

from ..module_utils.module_executor import ModuleExecutor

RESOURCE_NAME = os.path.basename(__file__).replace(".py", "")


def main():
    executor = ModuleExecutor(RESOURCE_NAME)
    executor.main()


if __name__ == "__main__":
    main()
