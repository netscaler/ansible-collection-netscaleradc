...
def main():
    ...
    lbvserver_service_binding_proxy = netscaler.ConfigProxy(actual, client, module_specific_arguments, module, read_only_attrs=read_only_attrs)
    try:
        if module.params['state'] == 'enabled':
            lbvserver_service_binding_proxy.add()
            print('I did it')
        elif module.params['state'] == 'disabled':
            lbvserver_service_binding_proxy.delete()
            print('I did it')
