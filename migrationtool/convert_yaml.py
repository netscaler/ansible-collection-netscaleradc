...
def get_task_level_keys(task_data, verbose):
    ...
    try:
        ...
        for key in task_data:
            ...
            elif isinstance(key, str) and key.split('.')[-1] not in resource_map and key not in ["citrix_adc_nitro_request"]:
                print(f"key: {key} is invalid in {task_level_keys['name']}")
    except Exception as e:
        print(f"Error processing task data: {e}")
        return {}
    if verbose:
        print(f"Final task level details for {task_level_keys['name']}: {task_level_keys}")
